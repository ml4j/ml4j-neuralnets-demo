...
public class ClassifierDemo
		extends SupervisedNeuralNetworkDemoBase<SupervisedFeedForwardNeuralNetwork, FeedForwardNeuralNetworkContext> {
	...
	@Override
	protected SupervisedFeedForwardNeuralNetwork createSupervisedNeuralNetwork(int featureCount,
		...
		DirectedComponentFactory directedComponentFactory = new DefaultDirectedComponentFactoryImpl(matrixFactory,
				axonsFactory, activationFunctionFactory, directedComponentsContext);

		FeedForwardLayer<?, ?> firstLayer = new FullyConnectedFeedForwardLayerImpl("FirstLayer",
				directedComponentFactory, axonsFactory,
				new AxonsConfig<>(new Neurons3D(28, 28, 1, true), new Neurons3D(20, 20, 1, false)),
				new DefaultSigmoidActivationFunctionImpl(), null);

		FeedForwardLayer<?, ?> secondLayer = new FullyConnectedFeedForwardLayerImpl("SecondLayer",
				directedComponentFactory, axonsFactory,
				new AxonsConfig<>(new Neurons3D(20, 20, 1, true), new Neurons(10, false)),
				new DefaultSoftmaxActivationFunctionImpl(), null);

		return new SupervisedFeedForwardNeuralNetworkImpl("NeuralNetwork", directedComponentFactory, firstLayer,
				secondLayer);
	}
	...
	protected NeuronsActivation createTrainingDataNeuronActivations(MatrixFactory matrixFactory) {
		...
		DoubleArrayMatrixLoader loader = new DoubleArrayMatrixLoader(ClassifierDemo.class.getClassLoader());
	...
	protected FeedForwardNeuralNetworkContext createTrainingContext(
			SupervisedFeedForwardNeuralNetwork supervisedNeuralNetwork,
			DirectedComponentsContext trainingDirectedComponentsContext) {
		...
		FeedForwardNeuralNetworkContext context = new FeedForwardNeuralNetworkContextImpl(
				trainingDirectedComponentsContext, true);
	...
	@Override
	protected void showcaseTrainedNeuralNetworkOnTrainingSet(SupervisedFeedForwardNeuralNetwork neuralNetwork,
			NeuronsActivation testDataInputActivations, NeuronsActivation testDataLabelActivations,
			DirectedComponentsContext showcaseDirectedComponentsContext) throws Exception {

		FeedForwardNeuralNetworkContext accuracyContext = new FeedForwardNeuralNetworkContextImpl(
				showcaseDirectedComponentsContext, false);

		double classificationAccuracy = neuralNetwork.getClassificationAccuracy(testDataInputActivations,
	...
	@Override
	protected void showcaseTrainedNeuralNetworkOnTestSet(SupervisedFeedForwardNeuralNetwork neuralNetwork,
			NeuronsActivation testDataInputActivations, NeuronsActivation testDataLabelActivations,
		...
		/*
		 * // Create a context for the first layer only FeedForwardNeuralNetworkContext
		 * autoEncoderNeuronVisualisationContext = new
		 * FeedForwardNeuralNetworkContextImpl(matrixFactory, 0, 0);
		 * 
		 * DirectedLayerContext hiddenNeuronInspectionContext =
		 * autoEncoderNeuronVisualisationContext.getLayerContext(0);
		 * 
		 * LOGGER.
		 * info("Drawing visualisations of patterns sought by the hidden neurons...");
		 * for (int j = 0; j < neuralNetwork.getFirstLayer().getOutputNeuronCount();
		 * j++) { NeuronsActivation neuronActivationMaximisingActivation =
		 * neuralNetwork.getFirstLayer() .getOptimalInputForOutputNeuron(j,
		 * hiddenNeuronInspectionContext); double[] neuronActivationMaximisingFeatures =
		 ...
		 * for (int i = 0; i < intensities.length; i++) { double val =
		 * neuronActivationMaximisingFeatures[i]; double boundary = 0.02; intensities[i]
		 * = val < -boundary ? 0 : val > boundary ? 1 : 0.5; }
		 * MnistUtils.draw(intensities, display); Thread.sleep(100); }
		 * 
		 */
		...
		for (int i = 0; i < 100; i++) {
			...
			NeuronsActivation orignalActivation = new NeuronsActivationImpl(new Neurons(activations.getRows(), false),
					activations, NeuronsActivationFormat.ROWS_SPAN_FEATURE_SET);

			MnistUtils.draw(orignalActivation.getActivations(directedComponentsContext.getMatrixFactory()).getRowByRowArray(), display);

			// Encode only through all Layers
			FeedForwardNeuralNetworkContext classifyingContext = new FeedForwardNeuralNetworkContextImpl(
					directedComponentsContext, false);

			ForwardPropagation forwardPropagtion = neuralNetwork.forwardPropagate(orignalActivation,
					classifyingContext);

			float[] values = forwardPropagtion.getOutput().getActivations(directedComponentsContext.getMatrixFactory()).getRowByRowArray();
